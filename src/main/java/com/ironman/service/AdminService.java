...
public class AdminService {
  ...
  public AssignmentResponse assign(UUID orderId, AssignmentRequest request) {
    ...
    // ── FIX: pickup only allowed after admin confirms the order ───────────────
    if (request.assignmentType() == AssignmentType.pickup) {
      if (order.getStatus() == OrderStatus.pending) {
        throw new BadRequestException(
                "Please confirm the order before assigning a pickup agent.");
      }
      if (order.getStatus() != OrderStatus.confirmed) {
        throw new BadRequestException(
                "Pickup can only be assigned when order is 'confirmed'. " +
                        "Current status: " + order.getStatus());
      }
    }

    // ── delivery only after order is ready ────────────────────────────────────
    if (request.assignmentType() == AssignmentType.delivery
            && order.getStatus() != OrderStatus.ready) {
      throw new BadRequestException(
              "Delivery can only be assigned when order is 'ready'. " +
                      "Current status: " + order.getStatus());
    }
    ...
    if (request.assignmentType() == AssignmentType.pickup) {
      orderService.updateStatus(order, OrderStatus.pickup_assigned,
              staff.getFullName() + " is coming to pickup", admin);
    }
    if (request.assignmentType() == AssignmentType.delivery) {
      orderService.updateStatus(order, OrderStatus.delivery_assigned,
              staff.getFullName() + " will deliver soon", admin);
    }

    // In-app notification
    notificationService.notifyUser(staff,
            "New assignment",
            "You have a " + request.assignmentType().name().replace('_', ' ')
                    + " task for " + order.getOrderNumber(),
            "assignment", assignment.getId());

    // Rich email directly to the staff member
    emailService.sendStaffAssignment(
            staff.getEmail(), staff.getFullName(),
            request.assignmentType().name().replace('_', ' '),
            order.getOrderNumber(), request.notes());
  ...
  private void validateStaffRole(AssignmentType type, UserRole role) {
    ...
    if (!valid) throw new BadRequestException(
            "Staff role " + role + " cannot handle assignment type " + type);
  }
}
